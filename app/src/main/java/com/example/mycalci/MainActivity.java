...
public class MainActivity extends AppCompatActivity {
    ...
    public void equalBTN(View view) {
        ...
        result1 = !result.contains(".") ? result : result.replaceAll("0*$", "").replaceAll("\\.$", "");


        double res2 = Double.parseDouble(result1);
        String fresult = df.format(res2);
    ...
    public void sciBTN(View view) {

        switchActivities();
        int orientation = this.getResources().getConfiguration().orientation;
        if (orientation == Configuration.ORIENTATION_PORTRAIT) {
            Toast.makeText(getApplicationContext(), "Rotate Your Device For More Functions", Toast.LENGTH_SHORT).show();
        }
